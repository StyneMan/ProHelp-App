...
class _MessageRowState extends State<MessageRow> {
  ...
  void initState() {
    ...
    widget.data['users']?.forEach((element) {
      ...
      if (socket.disconnected) {
        socket.connect();
        socket.emit("setup", element);
      } else {
        socket.emit("setup", element);
      }
    });

    socket.emit("join chat", widget.data['id'] ?? widget.data['_id']);
  }
  ...
  build(BuildContext context) {
    return InkWell(
      onTap: () {
        debugPrint("CHAT ID >> ${widget.data['latestMessage']}");
          ...
          : SizedBox(
              ...
              child: Row(
                ...
                children: [
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          TextPoppins(
                            text:
                                "${guestData['bio']['firstname']} ${guestData['bio']['middlename']} ${guestData['bio']['lastname']}"
                                    .capitalize,
                            fontSize: 16,
                  ...
                  TextInter(
                    text: Constants.timeUntil(DateTime.parse(
